...
export function Header() {
  return (
    <header className="sticky top-0 z-20 w-full max-w-full overflow-hidden border-b border-white/10 bg-slate-950/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-1.5 sm:gap-4 px-2.5 py-2.5 sm:px-6 sm:py-4 lg:px-8 w-full min-w-0">
        <Link href="/" className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 sm:flex-initial" aria-label="Online Fetcher Tools home">
          ...
          <div className="min-w-0 truncate">
            <div className="truncate text-xs sm:text-lg font-semibold tracking-tight text-white">Online Fetcher Tools</div>
        ...
        <div className="flex items-center gap-1 sm:gap-2.5 shrink-0">
          ...
          <details className="relative">
            <summary className="flex cursor-pointer list-none items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2 py-1 text-xs text-slate-200 sm:px-4 sm:py-2 sm:text-sm">
              <span>Tools</span>
              <ChevronDown className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
            </summary>
            <div className="absolute right-0 top-10 z-30 grid w-48 sm:w-56 gap-1 rounded-2xl border border-white/10 bg-slate-900 p-2 shadow-2xl sm:top-12">
