...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: `${title} | ClipFetch`, description: `${title} — a fast, private ClipFetch tool.` };
}
...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(34,211,238,0.13),_transparent_34%),linear-gradient(180deg,#020817_0%,#0f172a_100%)] text-slate-100">
    <div className="mx-auto max-w-7xl px-4 pb-20 pt-10 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-4xl pb-10 pt-16 text-center"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300">ClipFetch tool</p><h1 className="mt-4 text-4xl font-semibold tracking-tight text-white sm:text-5xl">{title}</h1><p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-slate-400">{isYoutube ? "Analyze a public YouTube URL, choose a format, and download content you are permitted to use." : "A focused, privacy-friendly utility that processes your files through the ClipFetch backend."}</p></div>
